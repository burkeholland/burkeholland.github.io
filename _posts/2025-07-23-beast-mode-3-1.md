---
layout: post
image: assets/images/banners/beast-mode-3-1.jpg
...
Thinking is only part of increasing accuracy though. The other important part is forcing 4.1 to use the internet to get information. It needs all the context it can get. Even if it knows that something won't work, it will do it anyway. For instance, even if it knows that you are using [shadcn/ui](https://ui.shadcn.com/docs), it will act like it has no clue how to use that library and just make up its own components. 

The solution to this is to make it do what any good developer would do - use Google. VS Code ships a very powerful tool called [`fetch`](https://code.visualstudio.com/docs/copilot/copilot-tips-and-tricks) out of the box for the agent. Fetch actually uses a headless browser in the background and it returns web content as markdown. Beast Mode forces 4.1 to use this tool recursively to search the web and get information before it acts. 
...
[Give Beast Mode a spin](https://gist.github.com/burkeholland/88af0249c4b6aff3820bf37898c8bacf). Remember that it is quite an opinionated way of working, so tweak it to your liking. Give the instructions at the top of the gist a good read before you use it, so you understand how tools work in the front matter.
